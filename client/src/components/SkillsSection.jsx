    ...
    <section id="skills" className="py-20 px-4 bg-slate-900">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-4">Compétences Techniques</h2>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {Object.entries(skills).map(([category, items]) => (
            <div 
              key={category}
              className="bg-gradient-to-br from-slate-800 to-slate-900 p-8 rounded-2xl border border-indigo-500/20 hover:border-indigo-500/40 transition-all"
            >
              <h3 className="text-2xl font-bold text-indigo-400 mb-6">{category}</h3>
              <div className="flex flex-wrap gap-3">
                {items.map((skill) => (
                  <span 
                    key={skill} 
                    className="px-4 py-2 bg-slate-700/50 text-gray-200 rounded-lg text-sm border border-slate-600 hover:border-indigo-500/50 hover:bg-slate-700 transition-all cursor-default"
                  >
                    {skill}
                  </span>
                ))}
